...
function makeCurve(
  ...
  const cpx = mx + nx * len * k * sign
  const cpy = my + ny * len * k * sign
  ...
  return {
    ...
    keyframeAnimation: {
      ...
      keyframes: [
        ...
        { percent: 1, style: { lineDashOffset: -40 } }
      ]
    }
...
function updateFlowArcs() {
  ...
  const elements = [
    makeCurve(pCP, pHD, 0.24, '#27ae60', center), // 昌平 → 海淀
    makeCurve(pHD, pYH, 0.24, '#27ae60', center), // 海淀 → 余杭
    makeCurve(pYH, pCP, 0.24, '#27ae60', center)  // 余杭 → 昌平
  ]
